package com.lianlianpay.openapi.security;
 ...
 * 聚数平台签名
 */
public class LLianPayMultiPaymentSignature {

    private static Logger log = LoggerFactory.getLogger(LLianPayMultiPaymentSignature.class);
    private static LLianPayMultiPaymentSignature instance;

    private LLianPayMultiPaymentSignature() {

    }

    public static LLianPayMultiPaymentSignature getInstance() {
        if (null == instance)
            return new LLianPayMultiPaymentSignature();
        return instance;
    }
    ...
    public String sign(String prikeyvalue, String sign_str) {
        try {
            ...
            return com.lianlianpay.openapi.security.RSASign.getInstance().sign(prikeyvalue, hash);
        } catch (Exception e) {
    ...
    public boolean checkSign(String pubkeyvalue, String sign_str, String signed_str) {
        try {
            ...
            return com.lianlianpay.openapi.security.RSASign.getInstance().checksign(pubkeyvalue, hash, signed_str);
        } catch (Exception e) {
    ...
    public String localEncrypt(String sourceStr) {
        ...
        try {
            encryptStr = com.lianlianpay.openapi.security.RSASign.getInstance().encrypt(sourceStr, LLianPayConstant.LLianPayPublicKey);
